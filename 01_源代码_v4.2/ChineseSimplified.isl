...
UninstallUnsupportedVer=卸载日志文件 "%1" 的格式不受此版本卸载程序支持。无法卸载。
UninstallUnknownEntry=在卸载日志中遇到未知项（%1）
ConfirmUninstall=您确定要完全删除 %1 及其所有组件吗？
UninstallOnlyOnWin64=该安装只能在 64 位 Windows 上卸载。
OnlyAdminCanUninstall=该安装只能由具有管理员权限的用户卸载。
UninstallStatusLabel=正在从您的计算机中删除 %1，请稍候。
UninstalledAll=%1 已成功从您的计算机中删除。
UninstalledMost=%1 卸载完成。%n%n有些项目未能删除，您可以手动删除它们。
UninstalledAndNeedsRestart=要完成 %1 的卸载，必须重新启动计算机。%n%n是否现在重启？
UninstallDataCorrupted=文件 "%1" 已损坏。无法卸载。
; *** Uninstallation phase messages
ConfirmDeleteSharedFileTitle=删除共享文件？
ConfirmDeleteSharedFile2=系统表明以下共享文件已经不再被任何程序使用。您希望卸载程序删除这个共享文件吗？%n%n如果仍有程序正在使用该文件，而它被删除，这些程序可能无法正常工作。如果您不确定，请选择“否”。将该文件保留在系统中不会造成危害。
